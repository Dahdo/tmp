...
def button_callback(line, event):
    ...
    if current_image:
        ...
        if line == button_next:
            ...
            tornado.ioloop.IOLoop.current().add_callback(app.reverse_url("image", next_image))
            led_next.set_value(0)
        
        elif line == button_prev:
            prev_image = images[(index - 1) % len(images)]
            led_prev.set_value(1)
            tornado.ioloop.IOLoop.current().add_callback(app.reverse_url("image", prev_image))
